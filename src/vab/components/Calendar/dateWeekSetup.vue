    ...
    <div class="week_main">
      <div class="flex_center_align flex_start">
        <div
          ...
          :class="{ active: state != 1, white: !disabled, grey: disabled }"
          @click="selectAll"
        >
          <svg-icon
            v-if="state != 1"
            :icon-class="state == 3 ? 'dui' : 'minimize_line'"
            class="className box_disable"
          />
        </div>
        <div class="day_title week_name" style="margin-left: 6px">
          全選択 / 解除
        </div>
      </div>
              ...
              <div class="left_checkbox flex_colum">
                ...
                >
                  <div class="flex_center_align flex_start">
                    ...
                    <div class="day_title week_name">第{{ index + 1 }}週</div>
                  </div>
                </div>
              </div>
            </div>
...
<script>
  export default {
    ...
    methods: {
      ...
      dayState() {
        if (this.dayList.every((item) => item.active == true)) {
          this.state = '3'
        } else {
          if (this.dayList.some((day) => day.active == true)) {
            this.state = '2'
          } else {
            this.state = '1'
          }
