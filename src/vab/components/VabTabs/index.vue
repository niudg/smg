  ...
  <div class="vab-tabs">
    ...
    >
      <el-tab-pane
        ...
        :name="item.path"
      >
        <span slot="label" @contextmenu.prevent="openMenu($event, item)">
          ...
          <span class="label_span" :title="translateTitle(item.meta.title)">
      ...
      <template #dropdown>
        <el-dropdown-menu class="tabs-more">
          <el-dropdown-item command="closeOthersTabs">
            ...
            <span>
              {{ translateTitle('他のタブを閉じる') }}
            </span>
          </el-dropdown-item>
          <el-dropdown-item command="closeLeftTabs">
            <vab-icon icon="arrow-left-line" />
            <span>
              {{ translateTitle('左のタブを閉じる') }}
            </span>
          </el-dropdown-item>
          <el-dropdown-item command="closeRightTabs">
            <vab-icon icon="arrow-right-line" />
            <span>
              {{ translateTitle('右のタブを閉じる') }}
            </span>
          </el-dropdown-item>
          <el-dropdown-item command="closeAllTabs">
            <vab-icon icon="close-line" />
            <span>
              {{ translateTitle('すべてのタブを閉じる') }}
            </span>
          </el-dropdown-item>
        </el-dropdown-menu>
      </template>
    ...
    >
      ...
      >
        <vab-icon icon="close-line" />
        <span>{{ translateTitle('他のタブを閉じる') }}</span>
      </li>
      <li
        class="el-dropdown-menu__item"
      ...
      >
        <vab-icon icon="arrow-left-line" />
        <span>{{ translateTitle('左のタブを閉じる') }}</span>
      </li>
      <li
        class="el-dropdown-menu__item"
        :class="{
      ...
      >
        <vab-icon icon="arrow-right-line" />
        <span>{{ translateTitle('右のタブを閉じる') }}</span>
      </li>
      <li class="el-dropdown-menu__item" @click="closeAllTabs">
        <vab-icon icon="close-line" />
        <span>{{ translateTitle('すべてのタブを閉じる') }}</span>
      </li>
    </ul>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      ...mapActions({
        ...
        setAutoReload: 'notify/setAutoReload',
      }),
      handleTabClick(tab) {
        console.info(tab.name)
        let paths = tab.name.split('/')
        this.setAutoReload({ name: paths[paths.length - 1], value: false })
